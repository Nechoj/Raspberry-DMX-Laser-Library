...
class Laser_Stairville__150_RGY: # for each laser model define such a class
    def __init__(self, offset=1): # most DMX devices allow for configuring an offset within the 512 DMX channels
       
        # parameter offset = DMX channel offset that can be configured at the device
        self.offset = offset-1 # self.offset is start index in buffer (0 = 1st byte)
...
class LaserManagement: # this class provides functions to deal with the connected lasers     
    def __init__(self):        
        ...
        self.Lasers = []
        L1 = Laser_Stairville__150_RGY(offset=1) # for each connected laser, instantiate such a class (use different offsets!) and append to Lasers[]
        self.Lasers.append(L1)
        #L2 = Laser_JB_Space_4(offset=10)
